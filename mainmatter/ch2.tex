...
\noindent
In the above examples we specified a desired colour by typing the name of a primary colour, but we are not limited to just using primary colours. R comes with a built in set of 657 differently named colours.  You can obtain the full list of colour names by running \R{colors()}.  R also has a built-in demo of these colours you can run to get a visual representation of each.  Simply run the command \R{demo("colors")}.\footnote{For some reason R spells ``colour'' incorrectly in these functions. This error has persisted for years, and the developers show no signs of correcting it.}

Alternatively, instead of typing a colour name, you can use a hexadecimal value (also referred to as a ``hex code'' or ``hex value'') that represents a specific colour. For example, the hex value \R{"\#FFC0CB"} represents the colour pink. Hexadecimal values offer the user a lot of nuance when it comes to colour selection and, in most cases, the simplest way of finding an appropriate hex value is to consult one of the many websites devoted to colour codes and colour theory (i.e., do an internet search). However, if you would like to understand the theory behind hex codes and why they are used, see Box \hyperlink{box:hex_codes}{2.2}.

\hypertarget{box:hex_codes}{}
...
\begin{inRhigh}[highlightlines={4}]
ggplot(msleep, aes(x = bodywt_log10, y = sleep_total)) +
  geom_point(size = 3, aes(colour = vore, shape = vore)) +
  geom_smooth(
    method = "lm", se = FALSE,
...
\begin{inRhigh}[highlightlines={8}]
ggplot(msleep, aes(x = bodywt_log10, y = sleep_total)) +
  geom_point(size = 3, aes(colour = vore, shape = vore)) +
  geom_smooth(
    method = "lm",
...
\begin{inRhigh}[highlightlines={7}]
ggplot(msleep, aes(x = bodywt_log10, y = sleep_total)) +
  geom_point(size = 3, aes(colour = vore, shape = vore)) +
  geom_smooth(
    method = "lm",
...
\begin{inRhigh}[highlightlines={9}]
ggplot(msleep, aes(x = bodywt_log10, y = sleep_total)) +
  geom_point(size = 3, aes(colour = vore, shape = vore)) +
  geom_smooth(
    method = "lm", 
...
Particularly for beginners with R, it is difficult to impress how useful \textit{ggplot2} is here. Using base R plotting functions to produce a comparable graph would be a considerably more complex process and require a heftier amount of code to be written, whereas \textit{ggplot2} does it all for us in four short lines.
...
To adjust the $x$ and $y$ axis titles we can simply use the function \R{labs()}.

\begin{inRhigh}[highlightlines={9-12}]
ggplot(msleep, aes(x = bodywt_log10, y = sleep_total)) +
  geom_point(size = 3) +
  geom_smooth(
    method = "lm",
...
A core concept in the ``grammar'' of \textit{ggplot2} is that of scales. Scales control how data is mapped to different aesthetics. For instance, there are scales for position, colour, size, shape, linetype, and so on. When we mapped an aesthetic to a variable—like we did above where we had mapped both colour and shape to the \R{\$vore} column:
...
\noindent
Thus, for the purpose of plotting, they are treated as a continuous scale and you would need to use \R{scale\_x\_continuous()} and \R{scale\_y\_continuous()} respectively. If we had, for instance, plotted a categorical variable on the $x$-axis (e.g., the conservation status of the animal) then the $x$-axis would be discrete while the $y$-axis remains continuous (we will see an example of this in Chapter 3 when we build a bar graph of skulls \emoji{metal}).

To customize the breaks on our axis, we simply need to add one of the aforementioned functions to our plot's code and provide a vector of values we want to see displayed using the argument \R{breaks}. For instance, if we want the x-axis to only display the numbers 1, 2, and 3, we would add \\ \R{scale\_x\_continuous(breaks = c(1,2,3))} to our code (see line 13).

\begin{inRhigh}[highlightlines={13}]
ggplot(msleep, aes(x = bodywt_log10, y = sleep_total)) +
  geom_point(size = 3) +
  geom_smooth(
    method = "lm",
...
In general, the best practice is not to specify values individually, but rather specify a sequence using the \R{seq()} function we learned about in Chapter 1 (see section \ref{sec:func_args}). For instance, we could have the $x$-axis increment by 1s and the $y$-axis increment by 2s (see lines 13 and 14).

\begin{inRhigh}[highlightlines={13-14}]
ggplot(msleep, aes(x = bodywt_log10, y = sleep_total)) +
  geom_point(size = 3) +
  geom_smooth(
    method = "lm",
...
\begin{inRhigh}[highlightlines={15}]
ggplot(msleep, aes(x = bodywt_log10, y = sleep_total)) +
  geom_point(size = 3) +
  geom_smooth(
    method = "lm",
...
\begin{figure}[H]
\centering
\includegraphics[scale = .3]{graphics/ch2Figs/col_discrete.png}
...
\begin{inRhigh}[highlightlines={6}]
ggplot(msleep, aes(x = bodywt_log10, y = sleep_total)) +
  geom_point(
    size = 3, shape = 21, stroke = 2,
    aes(colour = vore)
...
\begin{inRhigh}[highlightlines={3-4}]
ggplot(msleep, aes(x = bodywt_log10, y = sleep_total)) +
  geom_point(
...
\begin{inRhigh}[highlightlines={7-12}]
...
\begin{inRhigh}[highlightlines={7-14}]
...
To retrieve the hex codes for a given palette (e.g., \R{"Inferno"}), you will need to specify not only the palette name but also the number of colours you want using the \R{n} argument. This determines the granularity of the palette—higher values yield a more finely graded colour spectrum. That said, for most practical purposes, distinctions beyond 4 or 5 colours tend to become visually negligible with the sequential and diverging palettes. Visual examples of all three HCL palette types can be found in Appendix \ref{sec:AppendixPalettes}.
...
\begin{inRhigh}[highlightlines={6-10}]
ggplot(msleep, aes(x = bodywt_log10, y = sleep_total)) +
  geom_point(
    size = 3, shape = 21, stroke = 1, colour = "black",
    aes(fill = log10(brainwt))
...
\begin{inRhigh}[highlightlines={4,6-7}]
ggplot(msleep, aes(x = bodywt_log10, y = sleep_total)) +
  geom_point(
...
\begin{inRhigh}[highlightlines={6-7}]
ggplot(msleep, aes(x = bodywt_log10, y = sleep_total)) +
  geom_point(
    size = 3, stroke = 1, colour = "black",
    aes(fill = vore, shape = vore)
...
\begin{inRhigh}[highlightlines={8-11}]
ggplot(msleep, aes(x = bodywt_log10, y = sleep_total)) +
  geom_point(
    size = 3, stroke = 1, colour = "black",
    aes(fill = vore, shape = vore)
...
\begin{inRhigh}[highlightlines={1}]
...
The quickest way to modify the overall appearance of your plot - which works well as a starting point for other modifications you want to make - is to use one of \textit{ggplot2's} built in themes shown in Figure \ref{fig:gg_built-in-themes}.  Simply add the theme's function to your plot's code.  For instance, if you wanted to use the black and white theme, \R{theme\_bw()}, you would run ...
...
\noindent
Additional pre-built themes can be accessed via other R packages, such as \textit{ggthemes}.
...
\begin{inRhigh}[highlightlines={3,5}]
my_plot + theme_bw() +
  theme(
...
\begin{inRhigh}[highlightlines={3,7}]
my_plot + theme_bw() +
  theme(
    axis.text.x = element_text(
      size = 18, face = "bold", colour = "red", angle = 45,
...
\begin{enumerate}
    \item How do you change the \textit{order} of the categories?  For instance, suppose we wanted ``herbi'' to be at the top of the ``Diet'' legend. Or suppose we wanted it to come first in our sequence of faceted plots we created in section \ref{sec:facets}. How can we make that happen?
    
    \item  How do we adjust the \textit{names} of the categories? Each category of vore/diet was labelled using the shortened version contained in the data set, but what if we wanted to write out each category in its entirety? E.g., display ``carnivore'' instead of ``carni'', and ``herbivore'' instead of ``herbi'', and so on.
\end{enumerate}

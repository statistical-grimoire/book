...
\section{Why a Programming Language?}
...
At this point, it is worth addressing a question that comes up often: ``\textit{Why use R instead of something like Python or Julia?}'' It is a fair question. After all, Python and Julia—like R—are full-fledged programming languages that are powerful and capable, but the difference comes down to origin story.
...
\noindent
Additional packages, like Python's excellent \textit{numpy} package can simplify what needs to be written, but it still is not quite so good as what R offers as a baseline user experience.
...
\noindent
However, Julia still expects you to opt in to statistics, with basic functions like \R{mean()} not being available until you explicitly load them. That is not a flaw—it’s a philosophical choice. Julia gives you a lean, high-performance core, and leaves the rest to you.
...
\begin{figure}[t]
\centering
\fbox{\includegraphics[width = \textwidth]{graphics/ch1Figs/R_studio.pdf}}
...
\input{tables/ch-1/key_shortcuts.tex}
...
\begin{mdframed}[nobreak = true, style = miscFrame, frametitle = \Large\bfseries\IMFellEnglish Box 1.1: Are you using your keyboard properly?]
\IMFellEnglish
...
What happens if you take the square root of a negative number?\footnote{Recall that exponents can be used to take the square-root of a number.  For example, $\sqrt{4}$ can be expressed as $4^\frac{1}{2}$.}
...
Apart from the last two, R programmers tend to use all of these with seeming abandon. It is worth noting that different style guides for R have been developed and altered over the years with varying degrees of adoption. Presently there is no consensus on which style-guide should act in an official capacity for R; however, the most popular, and widely respected, is the \textit{Tidyverse} Style Guide\footnote{The \textit{tidyverse} will be explained in the next chapter, just know that all the code written in this book will (do its best) to adhere to its standards.} (\url{https://style.tidyverse.org}) which advocates the strict and concise use of \textit{snake\_case} only.

When it comes to naming objects, all of the rules just laid out only apply to what are referred to as \glspl{syntactic name}; however, if villainy is more your style, you can gleefully ignore all of those rules and conjure up what are called \glspl{non-syntactic name} by simply enclosing the name within backticks.
...
\begin{mdframed}[nobreak = true, style = miscFrame, frametitle = \Large\IMFellEnglish Box 1.2: How to Use Your Colon Effectively]
...
\noindent This can also be used in reverse and with negative values.
...
\input{tables/ch-1/operator_tab.tex}
...
\input{tables/ch-1/data_frame_ex.tex}
...
\noindent
Notice in the above example that we assigned four values to the \R{a} column and two values to the \R{b} column and instead of producing an error, R simply recycled the values in \R{b} to fill the empty spots.
...
R users tend to be on the fence as to whether this is a useful feature or not.\footnote{If you, like the \textit{tidyverse} high council, see this as a mild heresy, fear not—the \textit{tibble} (covered in Chapter 3) was made with you in mind.}  It does provide a nice visual confirmation that specific rows have been removed, but it makes future indexing potentially more confusing since the row named 10 is actually the 9\textsuperscript{th} row.  Thus, its often helpful to rename the rows after you have subset or removed certain values. You can do this using the \R{rownames()} function.
...
\begin{itemize}
    ...
    \begin{enumerate}
    \ttfamily\setlength\itemsep{-1em}
        \item In the Windows search bar type \enquote{File Explorer Options.}
...
\clearpage
    ...
    \ttfamily\setlength\itemsep{-1em}
        ...
        \item Open \textit{Settings} (\enquote{\textit{Preferences}} on older Macs)
...
Any time R needs to access or create a file, it needs to access or create that file somewhere and if you do not tell R where that somewhere is, it will default to what is known as the \gls{working directory}. To see where your current working directory is set to you can just run the function \R{getwd()}.

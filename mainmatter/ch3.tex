...
\noindent
It includes measurements of 150 ancient Egyptian skulls spanning five distinct historical periods: the Early Pre-Dynastic Period (\textit{circa} 4000 BCE), the Early Dynastic Period (\textit{circa} 3300 BCE), the Middle Kingdom (\textit{circa} 1850 BCE), the Ptolemaic Period (\textit{circa} 200 BCE), and the Roman Period (\textit{circa} 150 CE). The measurements, taken in millimetres\footnote{For any Americans reading this, 1 millimetre is 0.118 barleycorns.} (mm), represent the maximal breadth of the skull.\footnote{This is not necessarily a statistic anyone should care about, but Ancient Egypt is really cool and skulls are metal AF. }

Upon opening the file within GitHub,\footnote{\url{https://github.com/statistical-grimoire/book/blob/main/data/ch-3/Egyptian-skulls/Max_Breadth_TRM_1905.csv}} you will see that it displays the file's contents in a fairly typical spreadsheet layout (see Table \ref{tab:skulls} for an example displaying the first 10 rows).

\vspace{1em}

\input{tables/ch-3/skulls.tex}
...
...
\end{raw}
\caption*{Example of the \R{Max\_Breadth\_TRM\_1905.csv} data file displayed in its raw text format. Only the first ten rows are shown.}
\end{listing}
...
...
\end{raw}
\caption*{Example of the \R{Max\_Breadth\_TRM\_1905.csv} data file displayed in its raw text format if it were a .TSV file. Only the first ten rows are shown.}
\end{listing}
\vspace{1em}
...
You can now subset and manipulate the data frame \R{skulls} like was done in Chapter 1 when we first discussing data frames (see section \ref{sec:data_frames}). For instance, if we wanted to look at the mean number of green M\&M's across all the packages we could simply run:

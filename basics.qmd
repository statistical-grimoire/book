...
Often, when using Linux, in order to install specific R packages, additional software needs to be installed through the Linux terminal.  As an example, if you install the excellent "<a href = "https://fs.r-lib.org/index.html" target = "_blank">fs</a>" package for R: `install.packages("fs")`

The installation will likely fail and you will be prompted somewhere in the R console to install "libuv1-dev."  You would then need to open a terminal (or use the one RStudio or Positron provide) and type `sudo apt install libuv1-dev` to install (if you are on a Debian/Ubuntu-based distribution). Then retry the installation: `install.packages("fs")`

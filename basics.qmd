...
What lies ahead in this and the next two chapters is, to be blunt, nothing short of a trial by fire. These chapters contain a wealth of information that, at a quick glance, will probably feel overwhelming to novice programmers who are fond of skimming through the text (as opposed to reading it properly). Be forewarned, if you approach these chapters from the perspective that you need to memorize all their content, you will surely be consumed by the flames you are stepping in to. The goal here is not to turn you into an expert in R, programming, or statistics overnight. Instead, this and the subsequent two chapters are designed to immerse you, the reader, in the R language (and a small amount of statistics), so you can get some hands-on experience while building a strong foundational understanding. Your focus at this point should be on replicating the examples, experimenting, and trying to grasp the underlying logic of the code, not committing anything to memory (that will happen effortlessly overtime). 

These chapters also assume you have installed RStudio or Positron, although nearly everything we do will also work in the base-R environment that gets installed on Windows and macOS with the R language.
...
#| output: "asis"
#| echo: false
qs <- as.list(list.files("rexams/basics/creating_objects"))
exams2forms(qs, n = length(qs), edir = "rexams/basics/creating_objects", solution = TRUE)
```
:::
...
<i class="bi bi-exclamation-octagon-fill"></i> **Advanced Topic** - New programmers should skip this section.
...
- Workspace files can become quite large depending on your data, making them slow to load and difficult to store or transfer. By contrast, an R script ([.R]{.mono} file) is just a plain-text document and remains small.
...
::: {.callout-warning}
## Two versions of the term "Workspace"

Rather annoyingly, the term "workspace" also has a second meaning in Positron. This meaning will be covered in detail later (see @sec-R_proj_posit_workspace). Just know that, to easily distinguish between them, this book will use the phrase "R Workspace" and "Positron Workspace" respectively.
...
## <i class="bi bi-incognito"></i> Hidden Files and Folders

In addition to hiding file extensions (see @sec-file_ext), operating systems will also commonly hide certain files and folders entirely. Typically these are system and configuration files that are best left alone. That being said, when programming with R, or any language, it is important to be aware of them and know how to view them when necessary.

On macOS and Linux, hidden files and folders are prefixed with a period. For example, if you create a plain-text file called "[notes.txt]{.mono}" and rename it to "[.notes.txt]{.mono}", the file will promptly vanish. The file is still intact and in the same location it was prior to the renaming, except that now there is just no visual indication of it.

Windows handles hiding differently. The dot prefix has no special meaning to the operating system; instead, you must right-click the file, select *Properties*, and check the *Hidden* attribute box. However, some cross-platform applications on Windows respect the dot convention and treat dot-prefixed files as hidden, even though Windows Explorer does not.

To view hidden items, you can access them through a terminal application or enable their display in your graphical file manager. The latter requires manual activation:

<i class="bi bi-windows"></i> **Windows 11**

1. Open File Explorer
2. Click the *View* button from the toolbar, select *Show*, then choose *Hidden items*.

<i class="bi bi-apple"></i> **macOS**

- Press [**Cmd + Shift + .**]{.mono} (period) in any Finder window to toggle hidden file visibility.

<i class="bi bi-tux"></i> **Linux**

- In most distributions, pressing [**Ctrl + H**]{.mono} will toggle the display of hidden files inside a graphical file manager.

:::


## R Projects vs. Positron Workspaces {#sec-R_proj_posit_workspace}

Both RStudio and Positron offer convenient features for handling your working directory;[^dir_help] though, they differ somewhat in their approach. RStudio employs what are referred to as "R Projects" and Positron makes use of "Workspaces". Both are core features that users should be aware of.
...
### Positron Workspaces {#sec-posit_workspace}
...
::: {.callout-warning}
## R's Workspace vs. Positron's Workspace

In @sec-R_workspaces we talked about "workspaces" in the context of [.Rdata]{.mono} files. That usage of the term "workspace" is distinct from Positron's use of the term here. In the former case, we are refererring to a specific file that gets saved; whereas, in the case of Positron we are referring to a folder/directory on your computer.
:::

...
The way to interpret what we are seeing is as as a *path*, or route to get to the directory called *Documents*. In the case of Windows, `C:/` represents the computer's hard drive and many computers will have more than one of these, so it is vital to know which one you are working in. By convention, Windows uses drive letters (`C:`, `D:`, etc.) to denote different hard drives on the system.

Within the hard drive is the directory called `Users` for both Windows and MacOS. We can tell that `Users` is a directory here and not a file because it is bounded by forward slashes, `/`, and has no file extension. When it comes to directory paths, it is not uncommon to also see them written using backslashes (`\`), particularly on Windows. The reasons for this difference in convention boil down to the development history of various types of software. All you need to know is that R will always use a forward slash `/` when listing paths.

Continuing down the path, next we have a subdirectory of `Users` that is called `YourName`. When you set up a user account `YourName` will be typically populated with whatever user name you chose for your account.
...
#| echo: false
library(knitr)
dir.create("~/Desktop/tmp/Desktop/", recursive = TRUE)
opts_knit$set(root.dir = "~/Desktop/tmp/Desktop")
```
...
```{r}
#| echo: false
list.files(path = "~/Desktop/tmp/Desktop")
```
...
With respect to R studio a core tool users should be aware of are what are called "R projects."  An R project is, in essence, a folder stored on your computer (in a location of your choosing) that contains a [.Rproj]{.mono} file that stores settings specific to that project. Moreover, Rstudio can be directly launched using the [.Rproj]{.mono} file. This will automatically set the working directory to the project folder, allowing you to use relative file paths (like `"homework_1/myfile.csv"`) instead of absolute paths (like `"C:/Users/YourName/Documents/stats_class/homework_1/myfile.csv"`). Using projects will also help keep your work organized since all scripts, data files, and outputs stay together in one place. Taken together, all these features make your code work much more seamlessly when you need to share it with others or move it to a different computer.
...
Each time you open a project, RStudio will start a new R session, set your working directory to the project directory, and load your command history from that project (which is stored as a hidden file called [.Rhistory]{.mono}). This means you can immediately start working with relative file paths without needing to use setwd() or remember where your files are located.
  ...
  <!-- Improve R project intro. Add info about .Rhistory and workspaces and Positron's equivalent of R projects-->
...
name=bivariate data,
description={Data consisting of a single \gls{response variable} and one other related variable.}
}
...
description={A type of data analysis that uses assumptions to make generalizations, predictions, or decisions that extend beyond what a descriptive analysis of the data alone can show.}
}
...
description={A \gls{variable} that represents non-numeric characteristics or categories. These categories can be either ordered (e.g., letter grades) or unordered (e.g., handedness), but they must be mutually exclusive, meaning each observation can belong to only one category at a time.}
}

...
![The MS-DOS Prompt application in Windows 95, which allowed users to access the DOS command line that remained the foundation of the operating system.](imgs/run_R/win_95_MS_DOS_terminal.png){width=100% #fig-win95_MS_DOS fig-alt="MS-DOS Prompt application in Windows 95"} 
...
To install R - both the language and environment simultaneously - simply go to the *R Project for Statistical Computing* website: [https://www.r-project.org/](https://www.r-project.org/) The site makes no apologies for its spartan '90s aesthetic, and we should all be so principled. Additionally, you'll encounter lots of technical jargon throughout the site's various pages, and as a newcomer to R and programming, some (maybe even a lot) of it will feel opaque. That's not a problem and you don't need to feel bad about it. Skim through it, absorb what clicks, maybe look up one or two things, and let the rest wash over you. Doing this builds familiarity with the landscape and overtime, as your skillset and vocabularly grows, the jargon will become more transparent.
...
### Linux Installation (Ubuntu and Mint)
Experianced users of Linux are not likely to need any instructions on how to install R on to their system; however, inexperianced users may need a helping hand. Obviously there are a pleathora of distinct Linux distributions (a.k.a. operating systems) out there and it is unrealisic to attempt to cover all of them. Consequently, this section will focus on two common beginner friendly distributions: Ubuntu and Linux Mint.

#### Ubuntu

Many Linux distros, like Ubuntu, have their own officially currated repositories that you can download R from. However, the version of R these store often lags quite a bit behind the offical files obtained from the CRAN. Ubuntu's package maintainers need time to test software, ensure it integrates well with other software on Ubuntu, and verify it works across different Ubuntu versions. This deliberate process keeps things stable but means you're not getting the latest version of R immediately.

By contrast, if you need the latest R version or specific recent features/bug fixes, you can add the R Project's own repository to Ubuntu's package manager configuration (`apt`).
...
Next we are going to install two helper programs, `software-properties-common` and `dirmngr`. The former provides tools for managing software repositories. The latter is background service that manages OpenPGP (GPG) keys and certificates. It's used to verify GPG signatures on packages and keys. 
...
Next we add the R Project's repository to your system's package sources so `apt` knows where to find R packages. To do this, you need to determine the codename of Ubuntu you are running.  Ubuntu names each new release of its operating system with an animal codename (focal = 20.04, jammy = 22.04, noble = 24.04, etc.)  To determine the code name just run `lsb_release -cs` in the terminal.
...
Linux Mint is based on Ubuntu, so the installation process is identical, with one exception.  Mint has it's own distinct codenames. This means in the steps above where we replace `noble`, we will need to make sure that we use the Ubuntu codename that your version of Mint is based on.  This can be obtained by running the following in the terminal.
...
Everything else about the Mint install, is the same as Ubuntu.
...
::: callout-caution
## Coming Soon:
- Installation Instructions for R, RStudio, Positron.

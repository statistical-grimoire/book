...
To install R - both the language and environment simultaneously - simply go to the *R Project for Statistical Computing* website: [https://www.r-project.org/](https://www.r-project.org/).


::: callout-caution
The site makes no apologies for its spartan '90s aesthetic, and we should all be so principled. Additionally, you'll encounter lots of technical jargon throughout the site's various pages, and as a newcomer to R and programming, some (maybe even a lot) of it will feel opaque. That's not a problem and you shouldn't feel bad about it. Skim through it, absorb what clicks, maybe look up one or two things, and let the rest wash over you. Doing this builds familiarity with the landscape and overtime, as your skillset and vocabularly grows, the jargon will become more transparent.
:::
...
Next we add the R Project's repository to your system's package sources so `apt` knows where to find R packages. To do this, you need to determine the codename of Ubuntu you are running.  Ubuntu names each new release of its operating system with a codename (focal = 20.04, jammy = 22.04, noble = 24.04, etc.)  To determine the code name just run `lsb_release -cs` in the terminal.
